...
public class GreenThumb {
	...
	public void onBonemeal(final BonemealEvent event) {
		if (event.ID == Block.cactus.blockID || event.ID == Block.reed.blockID) {
			...
			// count the lily pads in a 9x9 square centered on this one
			for (int dX = -4; dX < 5; dX++) {
				for (int dZ = -4; dZ < 5; dZ++) {
					...
					if (count > 11) { // 12 lily pads in a 9x9 square ~= 15% coverage
						return;
					}
				}
			}
			...
			// attempt to place a new lily pad in a 5x5 square centered on this one
			while (true) {
				int x = event.X + this.random.nextInt(5) - 2;
				int z = event.Z + this.random.nextInt(5) - 2;
